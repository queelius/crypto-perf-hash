...
% FIXED: The original formula had an extra factor of m. Expanding:
% 1 - p_j = (j-1)/m, p_j^2 = (m-j+1)^2/m^2, so
% Var(T_j) = ((j-1)/m) / ((m-j+1)^2/m^2) = m(j-1)/(m-j+1)^2.
\begin{equation}
\mathrm{Var}(T) = \sum_{j=2}^{m} \frac{(j-1) \cdot m}{(m-j+1)^2}.
\end{equation}
